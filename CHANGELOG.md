...
### Added
- Prompt capture requests now include the client-observed submission time, preserving the original turn timestamp when durable outbox delivery is delayed.
...
### Added
- Successful `/prism:setup` runs now include a unique `setup_run_id` in the best-effort setup-completion notification, allowing compatible backends to correlate each completion with the setup run that produced it while preserving compatibility with older backends.
