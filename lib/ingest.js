 ...
 * @param {{ client_event_id: string, prompt_id: string, tool_session_id?: string, response_text: string, elapsed_ms?: number, input_tokens?: number, output_tokens?: number, cache_read_tokens?: number, cache_creation_tokens?: number, model?: string, cost_usd?: number, response_operation_id?: string, host_prompt_id?: string }} payload
 * @param {{ deadline?: number }} [options]
 * @returns {Promise<{ status: number, body: string }>}
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = transport.request(url, {
      ...
      res.on('end', () => {
        ...
        settle(resolve, { status: res.statusCode, body });
      });
    });
