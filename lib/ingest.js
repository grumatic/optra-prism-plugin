...
// ─── Host observation payload shape guards (contract §3) ───
// Mirrors the wire contract closely enough to catch a locally-built payload
// that drifted from it; the server remains the actual authority.

function isBoundedString(value, minBytes, maxBytes) {
  return typeof value === 'string' && Buffer.byteLength(value, 'utf8') >= minBytes && Buffer.byteLength(value, 'utf8') <= maxBytes;
}

function isBoundedStringOrNull(value, maxBytes) {
  return value === null || isBoundedString(value, 1, maxBytes);
}
...
function isRfc3339String(value) {
  return typeof value === 'string' && Number.isFinite(Date.parse(value));
}

function isRfc3339StringOrNull(value) {
  return value === null || isRfc3339String(value);
}

const HOST_OBSERVATION_COMMON_FIELDS = [
  'schema_version', 'adapter_event_id', 'source_session_id', 'collector_version', 'host_version', 'observed_at',
];

function hasExactKeys(payload, extraFields) {
  const allowed = new Set([...HOST_OBSERVATION_COMMON_FIELDS, ...extraFields]);
  return Object.keys(payload).every((key) => allowed.has(key)) && allowed.size === Object.keys(payload).length;
}

function validHostObservationCommon(payload, wireSchemaVersion) {
  return payload
    && typeof payload === 'object'
    && !Array.isArray(payload)
    && payload.schema_version === wireSchemaVersion
    && typeof payload.adapter_event_id === 'string'
    && /^[a-f0-9]{64}$/.test(payload.adapter_event_id)
    && isBoundedString(payload.source_session_id, 1, 1024)
    && isBoundedString(payload.collector_version, 1, 64)
    && isBoundedStringOrNull(payload.host_version, 64)
    && isRfc3339String(payload.observed_at);
}

const HOST_OBSERVATION_VALIDATORS = {
  prompt_input_origin: (payload) => hasExactKeys(payload, [
    'host_prompt_id', 'prompt_client_event_id', 'hook_source', 'transcript_prompt_source',
    'transcript_origin_kind', 'transcript_is_meta', 'transcript_row_uuid', 'observation_basis',
  ])
    && isBoundedString(payload.host_prompt_id, 1, 1024)
    && isBoundedString(payload.prompt_client_event_id, 1, 1024)
    && isBoundedStringOrNull(payload.hook_source, 64)
    && isBoundedStringOrNull(payload.transcript_prompt_source, 64)
    && isBoundedStringOrNull(payload.transcript_origin_kind, 64)
    && isBoolOrNull(payload.transcript_is_meta)
    && isBoundedStringOrNull(payload.transcript_row_uuid, 128)
    && ['hook_field', 'transcript_row', 'both'].includes(payload.observation_basis)
    && (payload.observation_basis !== 'hook_field' || payload.hook_source !== null)
    && (payload.observation_basis !== 'transcript_row' || (
      (payload.transcript_prompt_source !== null || payload.transcript_origin_kind !== null)
      && payload.transcript_row_uuid !== null
    ))
    && (payload.observation_basis !== 'both' || (
      payload.hook_source !== null
      && (payload.transcript_prompt_source !== null || payload.transcript_origin_kind !== null)
      && payload.transcript_row_uuid !== null
    )),
  queued_input: (payload) => hasExactKeys(payload, [
    'attachment_row_uuid', 'source_uuid', 'parent_row_uuid', 'origin_kind', 'is_meta', 'command_mode',
    'prompt_text', 'untruncated_sha256', 'original_char_count', 'attached_under_host_prompt_id', 'host_timestamp',
  ])
    && isBoundedString(payload.attachment_row_uuid, 1, 128)
    && isBoundedStringOrNull(payload.source_uuid, 128)
    && isBoundedStringOrNull(payload.parent_row_uuid, 128)
    && isBoundedStringOrNull(payload.origin_kind, 64)
    && isBoolOrNull(payload.is_meta)
    && isBoundedStringOrNull(payload.command_mode, 64)
    && typeof payload.prompt_text === 'string'
    && /^[a-f0-9]{64}$/.test(payload.untruncated_sha256)
    && isNonNegativeInt(payload.original_char_count)
    && isBoundedString(payload.attached_under_host_prompt_id, 1, 1024)
    && isRfc3339StringOrNull(payload.host_timestamp),
  turn_interrupt_marker: (payload) => hasExactKeys(payload, [
    'marker_kind', 'target_host_prompt_id', 'interrupted_message_id', 'row_uuid', 'host_timestamp',
  ])
    && ['user', 'tool_use'].includes(payload.marker_kind)
    && isBoundedString(payload.target_host_prompt_id, 1, 1024)
    && isBoundedStringOrNull(payload.interrupted_message_id, 128)
    && isBoundedString(payload.row_uuid, 1, 128)
    && isRfc3339StringOrNull(payload.host_timestamp),
  stop_context: (payload) => hasExactKeys(payload, [
    'host_prompt_id', 'stop_ordinal', 'stop_hook_active', 'background_task_count', 'session_cron_count',
  ])
    && isBoundedString(payload.host_prompt_id, 1, 1024)
    && isNonNegativeInt(payload.stop_ordinal)
    && typeof payload.stop_hook_active === 'boolean'
    && isNonNegativeInt(payload.background_task_count)
    && isNonNegativeInt(payload.session_cron_count),
  session_end: (payload) => hasExactKeys(payload, ['reason', 'last_active_host_prompt_id'])
    && isBoundedString(payload.reason, 1, 64)
    && isBoundedStringOrNull(payload.last_active_host_prompt_id, 1024),
};

/**
 * POST /v1/host-observations/{kind} — capture one host observation occurrence.
 * The route and wire schema version come from the kind's own registration in
 * lib/host-observations.js; this function only guards the payload shape.
 * @param {'prompt_input_origin'|'queued_input'|'turn_interrupt_marker'|'stop_context'|'session_end'} kind
 * @param {object} payload the frozen host observation payload
 * @param {{ deadline?: number }} [options]
 * @returns {Promise<{ status: number, body: string }>}
 */
async function sendHostObservation(kind, payload, options = {}) {
  const { HOST_OBSERVATION_KINDS } = require('./host-observations');
  const config = HOST_OBSERVATION_KINDS[kind];
  const validator = HOST_OBSERVATION_VALIDATORS[kind];
  if (!config || !validator) throw new TypeError(`sendHostObservation requires a known kind, got: ${kind}`);
  if (!validHostObservationCommon(payload, config.wireSchemaVersion) || !validator(payload)) {
    throw new TypeError(`sendHostObservation requires a valid ${kind} payload`);
  }
  return post(config.route, payload, options);
}
