...
const NO_REALTIME_DATA = 'No realtime data yet for this session. The summary fills in after the first completed prompt.';
const NO_REALTIME_DATA_GUIDANCE = 'Complete one prompt first, then run `/prism:realtime` again.';
const LATEST_SESSION_GUIDANCE = 'This summary came from the most recent session on this machine because the current session has no completed turns yet.';
...
// Session ids recovered from runtime dirs, newest first. Record files carry
// their own sessionId, so no hash reversal is needed; the official
// readSummary() then performs the fence-major, reconciled read.
function sessionIdsByMtime() {
...
function latestSessionId() {
  return sessionIdsByMtime()[0] || null;
}
...
function noRealtimeDataOutput() {
  return `${NO_REALTIME_DATA}\n\n${NO_REALTIME_DATA_GUIDANCE}`;
}
...
async function realtimeStatus(argv, env) {
  ...
  if (!summary || !summary.contextHealth || !(summary.contextHealth.turnCount > 0)) {
    // Fall back to the most recent session that actually has completed turns,
    // not merely the newest directory (which may be an empty session where the
    // user only ran slash commands).
    for (const candidate of sessionIdsByMtime()) {
      if (candidate === sessionId) continue;
      const candidateSummary = readSummary(candidate);
      if (candidateSummary && candidateSummary.contextHealth && candidateSummary.contextHealth.turnCount > 0) {
        sessionId = candidate;
  ...
  const totalCost = formatCost(totals.cost, totals.unknownCost);
  lines.push(`  ${totals.unknownCost ? totalCost : `cost ${totalCost}`} · ${summary.contextHealth.turnCount} turns`);
  lines.push(
  ...
  if (totals.unknownCost) {
    lines.push('cost is unavailable: some usage came from models without a reviewed price.');
  }
  if (today && typeof today.narrative === 'string' && today.narrative.length > 0) {
    lines.push('', 'Today', today.narrative);
  }
  if (annotation) lines.push('', LATEST_SESSION_GUIDANCE);
  return lines.join('\n');
}

module.exports = { realtimeStatus, latestSessionId, noRealtimeDataOutput };

if (require.main === module) {
  realtimeStatus(process.argv.slice(2), process.env)
    .then((output) => process.stdout.write(`${output}\n`))
    .catch(() => process.stdout.write(`${noRealtimeDataOutput()}\n`));
}
