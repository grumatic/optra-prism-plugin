...
/**
 * Score a prompt via the ingest service (proxies to engine).
...
async function scorePrism(promptText) {
  if (!GCK_KEY || !INGEST_URL) {
    debug('SKIP: no GCK_KEY or INGEST_URL');
    return null;
  }

  const scoreUrl = `${INGEST_URL}/v1/intelligence/score-prompt`;
  debug(`POST ${scoreUrl}`);
  ...
  try {
    const response = await fetch(scoreUrl, {
      ...
      body: JSON.stringify({ prompt_text: promptText.slice(0, 2000) }),
