...
function enqueueDetailed(entry) {
  ...
  try {
    ...
    reapOrphanTemps(dir);
    if (!fs.existsSync(file) && !reservePendingSlot()) return { outcome: 'capacity_full' };
...
function isTerminalInvalidHostPrompt(result) {
  if (!result || result.status !== 400 || result.mediaType !== 'application/json') return false;
  const bodyBytes = Number.isSafeInteger(result.bodyBytes)
    ? result.bodyBytes
    : Buffer.byteLength(typeof result.body === 'string' ? result.body : '', 'utf8');
  if (result.bodyTruncated === true || bodyBytes > MAX_TERMINAL_RESPONSE_BODY_BYTES) return false;
  try {
    ...
    return body
      ...
      && body.error.code === 'invalid_host_prompt_id';
  } catch {
    return false;
  }
}
...
async function drain(sender, options = {}) {
  ...
  for (const entry of ordered) {
    ...
    if (entry.dependsOn && (
      ...
      || (pendingIds.has(entry.dependsOn) && !acknowledged.has(entry.dependsOn))
    )) continue;
...
async function deliverEntry(entry, sender, deadline) {
  try {
    ...
    if (isTerminalInvalidHostPrompt(result)) {
      const settled = settleTerminal(entry, true);
      return {
        id: entry.id,
        acked: false,
        terminal: settled.state === 'terminal',
        terminalReason: settled.state === 'terminal' ? 'invalid_host_prompt_id' : `terminal_${settled.state}`,
