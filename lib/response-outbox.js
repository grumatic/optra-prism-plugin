...
const { markKindUnsupported } = require('./host-observation-state');

function isHostObservationKind(kind) {
  return Object.prototype.hasOwnProperty.call(HOST_OBSERVATION_KINDS, kind);
}

const MAX_PENDING_ENTRIES = 512;
// Bound the aggregate spool independently of the per-entry cap.
const MAX_PENDING_BYTES = 128 * 1024 * 1024;
// Reserve 128 KiB around the maximum escaped body for envelope fields.
...
function validEntry(entry) {
  return entry
    ...
    && entry.id.length > 0
    && entry.id.length <= 512
    && (entry.kind === 'prompt' || entry.kind === 'response' || entry.kind === 'prompt_evidence' || entry.kind === 'gap' || isHostObservationKind(entry.kind))
    && entry.payload
...
// Order candidates without parsing potentially multi-MiB payloads.
...
function reservePendingSlot(incomingBytes) {
  ...
  // Use createdAt for eviction because failed attempts refresh file mtime.
  const parsed = files
  ...
  // response intent. `queued_input` carries the sole copy of a user's queued
  // instruction, so it shares the `prompt` eviction priority even though it
  // is still evidence-shaped (tombstoned on eviction, like prompt_evidence).
  const evictionPriority = (kind) => {
    if (kind === 'prompt_evidence') return 'prompt_evidence';
    if (isHostObservationKind(kind)) return HOST_OBSERVATION_KINDS[kind].evictionTier;
    return kind;
  };
  let evicted = 0;
  for (const evictPriority of ['prompt_evidence', 'prompt']) {
    for (const { file, size, entry } of parsed) {
      if (evicted >= requiredCountEvictions && !overByteBudget()) break;
      if (evictionPriority(entry.kind) !== evictPriority) continue;
      const isEvidenceLike = entry.kind === 'prompt_evidence' || isHostObservationKind(entry.kind);
      if (isEvidenceLike) {
        ...
        evicted += 1;
        totalBytes -= size;
        debug(`DROP outbox ${entry.kind} beyond cap: ${path.basename(file)}`);
      } else {
        try {
          fs.unlinkSync(file);
          evicted += 1;
 ...
 * @param {{ id: string, kind: 'prompt'|'response'|'prompt_evidence'|'prompt_input_origin'|'queued_input'|'turn_interrupt_marker'|'stop_context'|'session_end', payload: object, dependsOn?: string, createdAt?: string, promotion?: object, legacyPromotion?: object }} entry
...
const TERMINAL_REJECTION_CODES = new Set([
  ...
  'host_observation_invalid_json',
  'host_observation_schema_unsupported',
  'host_observation_invalid_identity',
  'host_observation_invalid_payload',
  'host_observation_request_too_large',
  'host_observation_exceeds_limit',
  'host_observation_unsupported_media_type',
]);
  ...
  try {
    ...
    const expectedStatus = body.error.code === 'prompt_evidence_identity_conflict'
      ...
      : [
        ...
        ? 413
        : ['prompt_evidence_unsupported_media_type', 'host_observation_unsupported_media_type'].includes(body.error.code)
          ? 415
          : 400;
...
// Best-effort delivery-attempt persistence for backlog demotion.
function bumpDeliveryAttempts(entry) {
  const file = entryFile(entry.id);
  // Re-check before rename; server dedup absorbs the remaining ack race.
...
async function drain(sender, options = {}) {
  ...
  // Parse candidates lazily under the same deadline used for delivery.
  ...
  // Prioritized entries bypass the backlog scan.
  ...
  for (const { file } of candidates) {
    ...
    else if (entry.kind === 'prompt_evidence' || isHostObservationKind(entry.kind)) evidence.push(entry);
    else prompts.push(entry);
  }

  const acknowledged = new Set();
  // Demote repeatedly failing entries behind fresh work without a timer.
  ...
  for (const entry of prioritizedEntries.filter((entry) => entry.kind === 'prompt_evidence' || isHostObservationKind(entry.kind)).sort(bySendPriority)) schedule(entry);
  ...
  for (const entry of ordered) {
    ...
    // A missing or corrupt dependency must not block a response forever.
    if (entry.dependsOn && (
...
async function deliverEntry(entry, sender, deadline) {
  try {
    ...
    // Old-server compatibility (contract §5.4): a 404/405 on a host-
    // observation route means this ingest deployment predates the kind. Mark
    // it unsupported for the session (best-effort; a lost mark only costs one
    // more terminal 404/405 round trip later) and settle terminal so it is
    // never retried.
    if (isHostObservationKind(entry.kind) && result && (result.status === 404 || result.status === 405)) {
      try { markKindUnsupported(entry.payload && entry.payload.source_session_id, entry.kind); } catch {}
      const settled = settleTerminal({ ...entry, terminalReason: 'server_unsupported' }, true);
      return {
        id: entry.id,
        acked: false,
        terminal: settled.state === 'terminal',
        terminalReason: settled.state === 'terminal' ? 'server_unsupported' : `terminal_${settled.state}`,
