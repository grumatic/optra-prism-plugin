 ...
 * The ingest service (public, authenticates Prism API keys) proxies to the engine (private).
 *
 * URLs resolved from config endpoint, with env and local config ingest overrides.
 *
 * Returns: { ok, reason, data }
 *   ok=true,  data=<response>        — success
 *   ok=false, reason="no_api_key"    — Prism API key not configured
 ...
 *
 * Optional ISO-8601 from/to query params scope the report to that period.
 * Without them the engine returns its default window (~30 days).
 */
async function quickReport({ from, to } = {}) {
  return post('/v1/insights/report/quick', {}, 60000, { from, to });
}
...
function get(path, params, timeoutMs = 3000) {
  if (!API_KEY) {
...
function post(path, data, timeoutMs = 30000, query = {}) {
  if (!API_KEY) {
