...
async function deliverOutboxEntry(entry, options = {}) {
  const result = await (entry.kind === 'prompt'
    ...
    : entry.kind === 'prompt_evidence'
      ...
      : isHostObservationKind(entry.kind)
        ? sendHostObservation(entry.kind, entry.payload, options)
      : sendResponse(entry.payload, options));
  if (entry.kind === 'prompt_evidence' || isHostObservationKind(entry.kind)) {
    return {
      ...result,
      ack: isExactPromptEvidenceAck(
        result,
        isHostObservationKind(entry.kind)
          ? entry.payload.adapter_event_id
          : entry.payload.client_event_id,
      ),
    };
  }
