...
function formatConfigSource(rawConfig, key) {
  ...
  return key === 'apiKey' || key === 'showRealtimeSummary' ? 'default' : 'not set';
}
...
function renderStatus(inputs) {
  ...
  if (otelStatus.ok) {
    ...
    lines.push('**Restart:** Restart Claude Code if apiKey or ingest_url changed since launch.');
  } else {
  ...
  let realtimeSummary;
  if (config.showRealtimeSummary === true) realtimeSummary = 'On';
  else if (config.showRealtimeSummary === false) realtimeSummary = 'Off';
  else realtimeSummary = `invalid value (${JSON.stringify(config.showRealtimeSummary)})`;

  lines.push(
    '',
    `**Ingest health endpoint:** ${healthStatus}`,
    `**Prompt capture:** ${promptCapture}`,
    `**Realtime summary setting:** ${realtimeSummary} (source: ${formatConfigSource(rawConfig, 'showRealtimeSummary')})`,
