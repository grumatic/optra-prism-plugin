...
// Frozen at the key set that shipped alongside the legacy Prism-owned "0"
// assistant-response opt-out. Used only to decide whether an existing "0" was
// written by Prism itself (see assistantResponseOptOutIsPrismOwned below).
// A key added to the projection in a later version must NEVER be added here:
// an upgrading installation cannot yet have that key, so requiring it would
// make the legacy-ownership test permanently unsatisfiable and strand the
// stale "0" forever instead of letting activation repair it.
...
// Settings precedence, lowest to highest — must match Claude Code's own
// user -> project -> local merge order used throughout this file.
...
// Claude Code falls back to OTEL_LOG_USER_PROMPTS when the assistant-response
// key is absent. A "0" is the old Prism projection only if every key in the
// frozen LEGACY_PROJECTION_OWNERSHIP_KEYS snapshot already matches the current
// projection; a standalone or otherwise diverged "0" is an explicit user
// opt-out. Shared by removeLegacyAssistantResponseOptOut (acts on it) and
// checkOtelSettings (reports on it), so both draw the same line.
...
// Decide whether OTEL_LOG_ASSISTANT_RESPONSES should be written into
// `targetScope`'s file this sync pass. Two distinct reasons to skip it, both
// meaning "there is a recognized '0' opt-out this write must not shadow":
//
//   1. Same scope: `targetEnv` (targetScope's own, post-cleanup env) already
//      carries it. removeLegacyAssistantResponseOptOut already removed it if
//      ownership made it look like stale Prism residue; anything still here
//      is meant to stay — but only for this single pass (see below).
//
//   2. Cross scope, lower precedence: `effectiveBefore` shows the opt-out is
//      currently in effect from a DIFFERENT scope with LOWER settings
//      precedence than `targetScope` (user < project < local). A value this
//      scope writes always wins that merge over a lower-precedence scope, so
//      writing "1" here would silently override the opt-out even though
//      targetScope's own file never held it — this scope cannot reach into
//      another scope's file to clean up a stale key there, so leaving this
//      key out is the only safe move.
//
// A recognized opt-out effective from a HIGHER-precedence scope is NOT
// skipped here: writing "1" into a lower-precedence scope never changes the
// merged outcome (the higher scope still wins), so it is harmless to write —
// but it also means sync alone can never repair that state. checkOtelSettings
// reports that case separately as a cross-scope conflict, not sync-fixable
// drift.
//
// Either way, exclusion here lasts only this one projection pass: once every
// other managed key is written (by this or an earlier sync at targetScope), a
// genuine standalone "0" at targetScope becomes indistinguishable from a
// Prism-owned one on the NEXT sync and is removed and reprojected as the
// explicit managed "1" then — this mirrors the pre-existing (HEAD) behavior
// for the legacy opt-out and is pinned by a two-sync test, not an accident.
...
function buildExpectedOtelEnv() {
  ...
  return {
    ...
    otelEnv: {
      ...
      // Explicit so a single-event export never depends on the SDK falling
      // back to OTEL_LOG_USER_PROMPTS for this flag's default.
      OTEL_LOG_ASSISTANT_RESPONSES: '1',
      OTEL_LOG_TOOL_DETAILS: '1',
      // Bounds one BatchLogRecordProcessor export toward staying under the
      // ingest service's 4 MiB request-body limit. Claude Code's OTel log
      // exporter is the standard OpenTelemetry SDK, which reads the
      // OTEL_BLRP_* family from the environment (confirmed both by Claude
      // Code's own docs, which point to the OpenTelemetry SDK spec for "full
      // configuration options" on the OTLP exporter, and by the installed
      // `claude` binary itself containing the literal OTEL_BLRP_* variable
      // names). It is not a hard guarantee: content-bearing attributes such
      // as prompt/tool-detail text can each reach
      // CLAUDE_CODE_OTEL_CONTENT_MAX_LENGTH (61440 UTF-16 units), and
      // worst-case UTF-8/JSON escaping (e.g. control characters becoming
      // 6-byte \u00XX escapes under OTEL_EXPORTER_OTLP_PROTOCOL=http/json)
      // can push a batch of pathological records past 4 MiB regardless of
      // batch size. The ingest service's own size check remains the final
      // gate; this cap only keeps a *typical* export well under it and
      // reduces how often that gate is hit.
      //
      // The processor is timer-driven (one export per OTEL_BLRP_SCHEDULE_DELAY
      // tick, default 5000ms), and the in-memory log queue silently drops
      // records on overflow with no diagnostic. Cutting only the batch size
      // (SDK default 512) without also cutting the delay would cut sustained
      // drain throughput by the same factor and risk exactly that silent
      // loss under load. 100/1000ms keeps throughput at the SDK default's
      // ~100 records/s (512 records / 5000ms) while still reducing a single
      // export's worst case by roughly 5x.
...
function syncOtelSettings({ scope, projectDir, dataDir } = {}) {
  ...
  const update = updateSettingsAtomic(file, (settings) => {
    ...
    // removeLegacyAssistantResponseOptOut mutates settings.env in place, so
    // re-read it: a standalone (non-Prism-owned) "0" survives that call and
    // must also survive this projection, per projectOtelEnv's contract.
    const envAfterCleanup = settings.env || currentEnv;
    // Read fresh, pre-write: needed to detect a recognized opt-out that is
    // currently effective from a DIFFERENT, lower-precedence scope than
    // targetScope (see projectOtelEnv) — targetScope's own env alone cannot
    // reveal that.
...
function checkOtelSettings({ projectDir, dataDir } = {}) {
  ...
  // The scope sync would target for this project, i.e. the only scope any
  // remediation this function suggests can actually write to.
  ...
  for (const [key, value] of Object.entries(expected.otelEnv)) {
    ...
    if (key === LEGACY_ASSISTANT_RESPONSES_KEY && env[key] === LEGACY_ASSISTANT_RESPONSES_VALUE) {
      // A standalone assistant-response opt-out is intentionally never
      // projected over (see projectOtelEnv) and must not report as drift.
      ...
      if (source && source !== installScope && sourceRank !== null && targetRank !== null) {
        if (sourceRank < targetRank) {
          // A lower-precedence scope holds it; sync will not (and structurally
          // cannot) write over a file it does not target. Honored, like the
          // standalone case above.
          continue;
        }
        // sourceRank > targetRank: the value comes from a HIGHER-precedence
        // scope than sync ever writes to for this project, so re-running
        // setup/config can never change the effective outcome — this is a
        // cross-scope conflict, not sync-fixable drift (mirrors helperConflict
        // in syncPluginVersionMetadata / status.js).
        assistantResponseConflict = { key, source, installScope };
        continue;
      }
      // source === installScope (or scope unknown): sync can act on this
      // directly next time it runs, so report it as ordinary fixable drift.
    }
    mismatches.push(key);
  }
  if (dataDir) {
