...
// Frozen ownership signature for the legacy Prism-managed opt-out.
// Later projection keys must not be added here.
...
// A matching frozen projection identifies Prism-owned legacy state; otherwise
// the value is treated as an explicit user opt-out.
...
// Do not shadow a recognized same-scope or lower-precedence opt-out.
...
function buildExpectedOtelEnv() {
  ...
  return {
    ...
    otelEnv: {
      ...
      // Keep typical exports below the ingest limit without reducing the
      // processor's approximate default throughput.
