...
function detectInstallScope(projectDir) {
  ...
  if (userSettings?.enabledPlugins?.[PLUGIN_ID] === true) return 'user';

  const projectSettings = readJson(path.join(dir, '.claude', 'settings.json'));
  if (projectSettings?.enabledPlugins?.[PLUGIN_ID] === true) return 'project';

  const localSettings = readJson(path.join(dir, '.claude', 'settings.local.json'));
  if (localSettings?.enabledPlugins?.[PLUGIN_ID] === true) return 'local';

  return null;
}

function otelScopeFromInstallScope(installScope) {
  if (installScope === 'user') return 'user';
  return 'project';
}
