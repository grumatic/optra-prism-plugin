 ...
 *               so it's the safe home for the Prism API key when the plugin is
 ...
 * in) because the OTEL_EXPORTER_OTLP_HEADERS value embeds the API key.
 *
 * Ingest URL is resolved by lib/config.js from env, local config, cache, then
 ...
 * vars (it shouldn't — that would mean a Prism API key got committed).
 */
  ...
  if (shared) {
    warnings.push(
      `OTEL vars found in ${projectSharedSettingsPath(projectDir)} (shared, likely checked in). ` +
        'This can leak your Prism API key — remove them manually.',
    );
  }
 ...
 * Refuses 'project-shared' — we never write the API key to a checked-in file.
 */
 ...
 *
 * Used by uninstall to guarantee no stale OTEL keys (or API key secrets in
...
/**
 * Scope-aware uninstall cleanup.
 *
 * Only removes entries for the current session's install scope:
 *   user scope  → user entry + user settings (local project installs preserved)
 *   local scope → current project entry + project settings (user install preserved)
 *
 * Marketplace registration is always preserved.
 *
 * Returns { cleaned: string[], remaining: string[] }.
 */
function cleanupRegistries({ extraProjectDirs } = {}) {
  ...
  // Determine which scope the current session uses.
  // Project-local match takes precedence over user scope.
  let currentScope = null;
  if (isArray) {
    for (const e of entries) {
      if (projectDir && e.projectPath && path.resolve(e.projectPath) === projectDir) {
        currentScope = e.scope || 'local';
        break;
      }
    }
    if (!currentScope) {
      for (const e of entries) {
        if (e.scope === 'user') { currentScope = 'user'; break; }
      }
    }
  }
  ...
  if (isArray && installed) {
    if (currentScope === 'user') {
      // Remove user-scope entries only; keep local entries
      const kept = entries.filter((e) => e.scope !== 'user');
      if (kept.length !== entries.length) {
        if (kept.length === 0) {
          delete installed.plugins[PLUGIN_ID];
        } else {
          installed.plugins[PLUGIN_ID] = kept;
        }
        writeJson(INSTALLED_PLUGINS, installed);
        cleaned.push('installed_plugins.json (user scope)');
      }
      // Report remaining local installs
      for (const e of kept) {
        if (e.projectPath) remaining.push(e.projectPath);
      }
    } else {
      // Remove current project entry only; keep user + other project entries
      const kept = entries.filter((e) => {
        if (!e.projectPath || !projectDir) return true;
        return path.resolve(e.projectPath) !== projectDir;
      });
      if (kept.length !== entries.length) {
        if (kept.length === 0) {
          delete installed.plugins[PLUGIN_ID];
        ...
        writeJson(INSTALLED_PLUGINS, installed);
        cleaned.push('installed_plugins.json (project scope)');
      }
      for (const e of kept) {
        remaining.push(e.scope === 'user' ? '(user scope)' : e.projectPath || '(unknown)');
      }
    }
  }

  // Clean settings based on scope
  if (currentScope === 'user') {
    // User scope: clean user settings + OTEL from user settings
    const userSettings = readJson(USER_SETTINGS);
    if (userSettings) {
      let changed = false;
      if (userSettings.enabledPlugins && userSettings.enabledPlugins[PLUGIN_ID]) {
        delete userSettings.enabledPlugins[PLUGIN_ID];
        changed = true;
      }
      if (userSettings.env) {
        for (const key of OTEL_KEYS) {
          if (key in userSettings.env) { delete userSettings.env[key]; changed = true; }
        }
        if (Object.keys(userSettings.env).length === 0) delete userSettings.env;
      }
      if (changed) {
        writeJson(USER_SETTINGS, userSettings);
        cleaned.push('user settings.json');
      }
    }
  } else {
    // Local/project scope: clean current project settings only
    const projDirs = new Set();
    if (projectDir) projDirs.add(projectDir);
    if (extraProjectDirs) {
      for (const d of extraProjectDirs) projDirs.add(path.resolve(d));
    }
    ...
    for (const dir of projDirs) {
      for (const file of [
        ...
        if (changed) {
          writeJson(file, settings);
          cleaned.push(`${dir} ${path.basename(file)}`);
        }
      }
    }
  }

  // If no installs remain, clean shared config (~/.prism)
  if (remaining.length === 0) {
    const prismDir = path.join(os.homedir(), '.prism');
    try {
      fs.rmSync(prismDir, { recursive: true, force: true });
      cleaned.push('~/.prism');
    } catch {}
  }
...
// node settings.js cleanup-registries
//   (removes prism entries from installed_plugins.json + known_marketplaces.json)
...
if (require.main === module) {
  ...
  if (action === 'cleanup-registries') {
    const extraDirs = args.projectDir ? [args.projectDir] : [];
    const result = cleanupRegistries({ extraProjectDirs: extraDirs });
    if (result.cleaned.length === 0) {
      console.log('[prism] No registry entries to clean');
    } else {
      console.log(`[prism] Cleaned: ${result.cleaned.join(', ')}`);
    }
