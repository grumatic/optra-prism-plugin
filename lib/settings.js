...
function removeLegacyAssistantResponseOptOut(settings, expectedOtelEnv) {
  ...
  // key is absent. Remove only the old Prism projection: every non-secret,
  // non-versioned OTEL value must still match the configured Prism projection.
  // A standalone or otherwise diverged "0" remains an explicit user opt-out.
  const prismOwned = LEGACY_PROJECTION_OWNERSHIP_KEYS.every((key) => (
    Object.prototype.hasOwnProperty.call(expectedOtelEnv, key)
      && env[key] === expectedOtelEnv[key]
  ));
  if (!prismOwned) return false;
...
function syncOtelSettings({ scope, projectDir, dataDir } = {}) {
  ...
  const update = updateSettingsAtomic(file, (settings) => {
    ...
    const envChanged = Object.entries(expected.otelEnv)
      .some(([key, value]) => currentEnv[key] !== value);
    settings.env = { ...currentEnv, ...expected.otelEnv };
    return {
      settings,
