...
function isGitValue(value) {
  return value
    && typeof value === 'object'
    && hasOnlyKeys(value, new Set([
      'schema_version', 'observed_at', 'host', 'owner', 'owner_path', 'repo',
      'branch', 'head', 'dirty', 'worktree', 'root_fingerprint', 'coverage', 'reason',
    ]))
    && value.schema_version === PROMPT_GIT_METADATA_SCHEMA_VERSION
    && timestampIsValid(value.observed_at)
    && ['host', 'owner', 'repo', 'branch'].every((key) => value[key] === null || typeof value[key] === 'string')
    && (value.owner_path === undefined || typeof value.owner_path === 'string')
    && typeof value.head === 'string'
    && /^[a-f0-9]{40,64}$/i.test(value.head)
    && typeof value.dirty === 'boolean'
    && typeof value.worktree === 'boolean'
    && (value.root_fingerprint === undefined || /^[a-f0-9]{64}$/.test(value.root_fingerprint))
    && [PROMPT_GIT_COVERAGE_READY, PROMPT_GIT_COVERAGE_UNAVAILABLE].includes(value.coverage)
    && (value.reason === undefined || PROMPT_GIT_REASONS.includes(value.reason));
}
...
function isGitRecord(value, sessionId) {
  return value
    ...
    && (
      value.reason === null
      || (typeof value.reason === 'string' && PROMPT_GIT_ENVELOPE_REASONS.includes(value.reason))
    )
    && (value.status !== 'ok' || value.reason === null);
}

const GIT_CAPTURE_UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

// The pending marker durably recording that a Stop turn owes a git-evidence/v1
// capture, written under the same fence as the turn record it accompanies —
// see publishAndConsumeActive. `baselineReason` is non-null only when the
// prompt-phase head could not be trusted as the range baseline (see the
// stop-handler's `observed_at <= submittedAt` guard).
function isGitCaptureRecord(value, sessionId) {
  return value
    && typeof value === 'object'
    && hasOnlyKeys(value, new Set([
      'schemaVersion', 'sessionId', 'updatedAt', 'generation', 'eventId', 'epoch',
      'clientEventId', 'submitPromptId', 'serverPromptId', 'responseOperationId',
      'canonicalCwd', 'baselineHead', 'baselineReason', 'createdAt',
    ]))
    && value.schemaVersion === SCHEMA_VERSION
    && value.sessionId === sessionId
    && timestampIsValid(value.updatedAt)
    ...
    && GIT_CAPTURE_UUID_PATTERN.test(value.eventId)
    && Number.isSafeInteger(value.epoch)
    && value.epoch >= 0
    && typeof value.clientEventId === 'string'
    && value.clientEventId.length > 0
    && typeof value.submitPromptId === 'string'
    && value.submitPromptId.length > 0
    && validServerPromptId(value.serverPromptId)
    && typeof value.responseOperationId === 'string'
    && value.responseOperationId.length > 0
    && (value.canonicalCwd === null || typeof value.canonicalCwd === 'string')
    && (value.baselineHead === null || /^[a-f0-9]{40,64}$/.test(value.baselineHead))
    && (value.baselineReason === null || value.baselineReason === 'baseline_missing')
    && timestampIsValid(value.createdAt);
}

function readGitCapture(sessionId) {
  return readRecord(sessionId, 'git-capture', isGitCaptureRecord);
}

/** Removes every generation of the git-capture marker for this session. */
function clearGitCapture(sessionId) {
  const result = withSessionLock(sessionId, (sessionDir) => {
    let entries;
    try {
    ...
    for (const entry of entries) {
      if (!entry.isFile() || !recordFileInfo('git-capture', entry.name)) continue;
      try {
...
function writeGit(sessionId, context) {
  const reason = context && Object.hasOwn(context, 'reason') ? context.reason : null;
  if (
    !context
    ...
    || !(reason === null || (typeof reason === 'string' && PROMPT_GIT_ENVELOPE_REASONS.includes(reason)))
    || (context.status === 'ok' && reason !== null)
  ) return null;
...
function promoteLegacyActive(sessionId, clientEventId, epoch, serverPromptId) {
  if (
    !clientEventId
  ...
  return withSessionLock(sessionId, (sessionDir, fence) => {
    ...
    if (
      ...
      || current.epoch !== epoch
      || current.active.clientEventId !== clientEventId
      || current.active.submitPromptId !== undefined
      || current.active.status !== 'submitting'
    ) return null;
    const next = {
      ...current,
...
function publishAndConsumeActive(sessionId, expected, publish) {
  ...
  return withSessionLock(sessionId, (sessionDir, fence) => {
    ...
    if (current.active.status === 'consumed') return { state: 'already_consumed' };
    if (current.active.status !== 'captured') return { state: 'not_current' };

    const publication = publish();
    if (!publication || publication.success !== true) {
      return { state: 'enqueue_failed', publication };
    }
    // A marker write failure is logged and ignored: it must never turn a
    // successful response publication into a failure. The turn-record write
    // below remains the sole authority for `published`.
    if (publication.gitCapture) {
      try {
        const currentCapture = readRecord(sessionId, 'git-capture', isGitCaptureRecord);
        const captureNext = {
          schemaVersion: SCHEMA_VERSION,
          sessionId,
          updatedAt: now(),
          generation: (currentCapture ? currentCapture.generation : 0) + 1,
          ...publication.gitCapture,
        };
        if (isGitCaptureRecord(captureNext, sessionId)) {
          writeRecord(
            sessionDir,
            'git-capture',
            captureNext,
            currentCapture ? currentCapture.generation : null,
            isGitCaptureRecord,
            sessionId,
            fence,
          );
    ...
    if (!writeRecord(sessionDir, 'turn', next, current.generation, isTurnRecord, sessionId, fence)) {
      return { state: 'state_write_failed', publication };
    }
...
function validResponseFence(fence) {
  if (
    !fence
    || !validSessionId(fence.sessionId)
    || !Number.isSafeInteger(fence.epoch)
    || fence.epoch < 0
    || typeof fence.clientEventId !== 'string'
    || fence.clientEventId.length === 0
    || fence.clientEventId.length > 1024
    || typeof fence.submitPromptId !== 'string'
    || fence.submitPromptId.length === 0
    || fence.submitPromptId.length > 1024
    || !validServerPromptId(fence.serverPromptId)
  ) return false;
  return true;
}

function responseFenceStatusForTurn(turn, fence) {
  if (!turn) return 'blocked_missing';
  if (turn.epoch > fence.epoch) return 'ready_invalidated';
  if (turn.epoch !== fence.epoch || !turn.active) return 'blocked_not_current';
  if (
    turn.active.clientEventId !== fence.clientEventId
    || turn.active.submitPromptId !== fence.submitPromptId
    || turn.active.serverPromptId !== fence.serverPromptId
  ) return 'blocked_not_current';
  if (turn.active.status === 'consumed') return 'ready_consumed';
  return turn.active.status === 'captured' ? 'blocked_captured' : 'blocked_not_current';
}

function sessionDirectoryState(sessionDir) {
  try {
    return fs.statSync(sessionDir).isDirectory() ? 'present' : 'unreadable';
  } catch (err) {
    return err && err.code === 'ENOENT' ? 'absent' : 'unreadable';
  }
}

function responseFenceStatus(fence) {
  if (!validResponseFence(fence)) return 'blocked_invalid';
  const sessionDir = getSessionDir(fence.sessionId);
  const directory = sessionDirectoryState(sessionDir);
  if (directory === 'absent') return 'ready_absent';
  if (directory !== 'present') return 'blocked_unreadable';
  const record = readRecordEntry(fence.sessionId, 'turn', isTurnRecord);
  if (!record) {
    const currentDirectory = sessionDirectoryState(sessionDir);
    if (currentDirectory === 'absent') return 'ready_absent';
    return currentDirectory === 'present' ? 'blocked_missing' : 'blocked_unreadable';
  }
  return responseFenceStatusForTurn(record.value, fence);
}

function recoverResponseFence(fence) {
  if (!validResponseFence(fence)) return 'blocked_invalid';
  const sessionDir = getSessionDir(fence.sessionId);
  const directory = sessionDirectoryState(sessionDir);
  if (directory === 'absent') return 'ready_absent';
  if (directory !== 'present') return 'blocked_unreadable';
  const recovered = withSessionDirectoryLock(sessionDir, (lockedSessionDir, lockFence) => {
    if (!reconcileCompactBarrier(fence.sessionId, lockedSessionDir, lockFence)) return 'blocked_reconcile';
    const record = readRecordEntry(fence.sessionId, 'turn', isTurnRecord);
    const status = record
      ? responseFenceStatusForTurn(record.value, fence)
      : (() => {
        const currentDirectory = sessionDirectoryState(lockedSessionDir);
        if (currentDirectory === 'absent') return 'ready_absent';
        return currentDirectory === 'present' ? 'blocked_missing' : 'blocked_unreadable';
      })();
    if (status !== 'blocked_captured') return status;
    const next = {
      ...record.value,
      updatedAt: now(),
      generation: record.value.generation + 1,
      active: { ...record.value.active, status: 'consumed' },
    };
    if (writeRecord(
      lockedSessionDir,
      'turn',
      ...
      fence.sessionId,
      lockFence,
    )) return 'ready_consumed';
    const current = readRecordEntry(fence.sessionId, 'turn', isTurnRecord);
    return current
      ? responseFenceStatusForTurn(current.value, fence)
      : (() => {
        const currentDirectory = sessionDirectoryState(lockedSessionDir);
        if (currentDirectory === 'absent') return 'ready_absent';
        return currentDirectory === 'present' ? 'blocked_missing' : 'blocked_unreadable';
