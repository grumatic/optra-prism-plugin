...
function getConfig(apiKey) {
  ...
  // Merge: env var overrides → cache → production fallbacks
  const gatewayUrl = envGateway || (cached && cached.gateway_url) || PROD_GATEWAY_URL;
  return {
    ingest_url: envIngest || (cached && cached.ingest_url) || PROD_INGEST_URL,
