...
const { getConfigDefaults, normalizeConfigFields } = require('./config-fields');
...
const DEFAULT_CONFIG = getConfigDefaults();
...
function writeConfig(config) {
  ...
  const normalized = normalizeConfigFields(config);
  const file = getConfigFile();
...
function getConfig() {
  return { ...DEFAULT_CONFIG, ...normalizeConfigFields(readConfig()) };
}

function removeConfigField(name, legacyNames = []) {
  const next = { ...readConfig() };
  delete next[name];
  for (const legacyName of legacyNames) delete next[legacyName];
