...
const AUTH_ERROR_CACHE_TTL_MS = 60 * 1000;
...
const CACHE_METADATA_KEYS = ['api_key_fingerprint', 'cached_at', 'source', 'auth_status'];
...
function getCachedConfig(apiKey) {
  try {
    ...
    const ttl = cache.source === 'auth-error'
      ? AUTH_ERROR_CACHE_TTL_MS
      : CACHE_TTL_MS;
 ...
 * Keeps credential rejection distinct from endpoint or network unavailability.
 */
async function fetchConfigResult(apiKey) {
  if (!isSupportedApiKey(apiKey)) return { status: 'invalid-key' };
  ...
  try {
    ...
    if (!config.ingest_url) return { status: 'unavailable' };
    ...
    return { status: 'server', config: cacheData };
  } catch {
...
async function fetchConfig(apiKey) {
  const result = await fetchConfigResult(apiKey);
  return result.status === 'server' ? result.config : null;
}
 ...
 * on failure, writes a cache populated with production fallback URLs. The
 * source differentiates endpoint unavailability from credential rejection.
 * Returns the cache object (never null).
 */
async function ensureCache(apiKey) {
  ...
  if (result.status === 'server') {
    return {
      ...result.config,
      ingest_url: resolveIngestUrl(result.config),
      source: 'server',
    };
  }
  ...
  const fallback = {
    ...
    source: result.status === 'auth-error' ? 'auth-error' : 'fallback',
    api_key_fingerprint: fingerprintApiKey(apiKey),
    cached_at: new Date().toISOString(),
  };
  if (result.status === 'auth-error') fallback.auth_status = result.authStatus;
