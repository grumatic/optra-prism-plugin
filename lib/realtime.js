...
function consumeUsage(usage, processedUsageIds, catalog) {
  ...
  for (const item of Array.isArray(usage) ? usage : []) {
    ...
    let rate = rateFor(catalog, item.model, item.occurredAt);
    if (!rate) {
      totals.unknownCost = true;
      continue;
    }
    // Above 200k input-side tokens a tiered model prices the whole request at
    // its premium rates; a tier without published rates stays unpriced.
    const inputSide = item.input + item.cacheRead + item.cacheCreation;
    if (inputSide > 200_000 && rate.hasLongContextTier) {
      if (!rate.longContext) {
        totals.unknownCost = true;
        continue;
      }
      rate = { ...rate.longContext, revision: rate.revision };
    }
